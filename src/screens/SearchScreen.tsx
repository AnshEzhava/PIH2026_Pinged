...
export default function SearchScreen() {
  const { selectedDisease, selectDisease, loading, error, clearError } = useAppContext();
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    if (query.length < 2) {
      getPopularDiseases().then(setDiseases).catch(() => {});
      return;
    }
    setSearching(true);
    try {
    ...
    >
      ...
      {error && (
        ...
        >
          ...
          <Text style={{ fontSize: 10, color: '#EF4444', marginTop: 2 }}>Tap to dismiss</Text>
        </Pressable>
      )}
      ...
      {loading.predict && (
        <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingTop: 8, gap: 6 }}>
          ...
          <Text style={{ fontSize: 11, color: MUTED_FG, fontWeight: '500' }}>Scoring candidates…</Text>
        </View>
      )}
      ...
      <View style={{ paddingHorizontal: 12, paddingVertical: 8 }}>
        ...
        >
          ...
          />
          {searching && <ActivityIndicator size="small" color={ACCENT} style={{ marginLeft: 6 }} />}
        </View>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          !searching ? (
            <Text style={{ textAlign: 'center', paddingVertical: 24, fontSize: 12, color: MUTED_FG }}>
              No diseases found
            </Text>
          ) : null
        }
      ...
      <View style={{ borderTopWidth: 1, borderTopColor: BORDER, paddingHorizontal: 12, paddingVertical: 8 }}>
        ...
        >
          <View>
            <Text style={{ fontSize: 12, fontWeight: '600', color: '#111827' }}>Advanced Input</Text>
            <Text style={{ fontSize: 10, color: MUTED_FG, marginTop: 1 }}>Molecular/genetic data</Text>
          </View>
          {advancedExpanded
        ...
        {advancedExpanded && (
          <View style={{ marginTop: 8 }}>
            <Text style={{ fontSize: 10, color: MUTED_FG, marginBottom: 4 }}>Disease ID (EFO/MONDO)</Text>
            <TextInput
              style={{
                borderWidth: 1,
                borderColor: BORDER,
